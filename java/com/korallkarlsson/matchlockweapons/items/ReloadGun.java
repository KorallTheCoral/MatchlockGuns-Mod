...
public class ReloadGun extends Item implements IHasModel {
		...
		public ReloadGun(String name, float inAccuracy, float damage, int maxShots, int cooldown)
		{
		...
		public ReloadGun(String name, float inAccuracy, float damage, int maxShots, int cooldown, double kickback)
		{
			setUnlocalizedName(name);
			setRegistryName(name);
			setCreativeTab(CreativeTabs.COMBAT);
		...
		public ReloadGun(String name, float inAccuracy, float damage, int maxShots, int cooldown, double kickback, double failRate)
		{
			setUnlocalizedName(name);
			setRegistryName(name);
			setCreativeTab(CreativeTabs.COMBAT);
		...
		public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
			...
			{
			if(playerIn.getHeldItemMainhand().getTagCompound().getInteger("loadedshots") > 0 && playerIn.getHeldItemMainhand().getTagCompound().getInteger("cooldown") <= 0)
			{
			...
			else if(!worldIn.isRemote && playerIn.getHeldItemMainhand().getTagCompound().getInteger("loadedshots") == 0)
			{
